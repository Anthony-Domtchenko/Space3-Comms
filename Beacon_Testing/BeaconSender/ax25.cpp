...
RxAx25::RxAx25(std::vector<char>& packet) {
  ...
  std::vector<char> fcsSlice(packet.begin() + 1, packet.end() - 3);
  calculatedFcs = calculateFcs(fcsSlice);
}
...
std::array<char,2> RxAx25::getCalculatedFcs() {
  return calculatedFcs;
}

bool RxAx25::fcsCompare() {
  if (getFcs()[0] != getCalculatedFcs()[0] || getFcs()[1] != getCalculatedFcs()[1]) {
    return false;
  }
  return true;
}
...
std::vector<char> ax25encode(std::vector<char>& data, bool msgType) {
  ...
  std::vector<char> fcsData;          // vector containing data for just the fcs calculation
  encodedPacket.reserve(300);
  ...
  fcsData.push_back(0x8E); // 'G'
  fcsData.push_back(0xA4); // 'R'
  fcsData.push_back(0x9E); // 'O'
  fcsData.push_back(0xAA); // 'U'
  fcsData.push_back(0x9C); // 'N'
  fcsData.push_back(0x88); // 'D'

  if (msgType == true) {
    encodedPacket.push_back(0xFC); // SSID = 0b1110 for WOD Data
    fcsData.push_back(0xFC);
  }
  else {
    encodedPacket.push_back(0xFE); // SSID = 0b1111 for Science Data
    fcsData.push_back(0xFE);
  }
  ...
  fcsData.push_back(0x88); // 'D'
  fcsData.push_back(0x9E); // 'O'
  fcsData.push_back(0x86); // 'C'
  fcsData.push_back(0x96); // 'K'
  fcsData.push_back(0x8A); // 'E'
  fcsData.push_back(0xA4); // 'R'
  fcsData.push_back(0xF7); // SSID = 0b1011 (11 in decimal) cos spacecraft

  // Add control field
  encodedPacket.push_back(0x03);
  fcsData.push_back(0x03);

  // Add protocol id
  encodedPacket.push_back(0xf0);
  fcsData.push_back(0xf0);
...
std::array<char,2> calculateFcs(std::vector<char>& data) {
  ...
  char highByte = crc >> 8;
  char lowByte = crc & 0xFF;
  std::array<char,2> fcs = {highByte, lowByte};
